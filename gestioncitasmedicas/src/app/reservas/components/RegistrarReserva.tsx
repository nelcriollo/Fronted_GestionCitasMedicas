...
export const RegistrarReservaComponent = ({ onNextStep, onPreviousStep
  ...
  const [reserva, setReserva] = useState<ReservaCita>({
    ...
    estado: 0,
    usuario: loginResponse ? {idUsuario:loginResponse.idUsuario} :{},
    horario: horarioSeleccionado ? { idHorario: horarioSeleccionado.idHorario } : {},
    paciente: pacienteRegistrado ? {idPaciente: pacienteRegistrado.idPaciente}: {},
  });
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={6}>
            <TextField
              ...
              onChange={(event) => setReserva({ ...reserva, precio: parseFloat(event.target.value) })}
              sx={{ marginBottom: '16px' }}
            />
          </Grid>
          <Grid item xs={6}>
            <TextField
              ...
              onChange={(event) => setReserva({ ...reserva, estado: parseInt(event.target.value) })}
              sx={{ marginBottom: '16px' }}
            />
          </Grid>
          <Grid item xs={6}>
          ...
          <Grid item xs={6}>
            <TextField
              ...
              onChange={(event) =>
                setReserva({ ...reserva, horario: { ...reserva.horario, idHorario: event.target.value } })
              }
              sx={{ marginBottom: '16px' }}
            />
          </Grid>
          <Grid item xs={6}>
            <TextField
              ...
              onChange={(event) =>
                setReserva({ ...reserva, paciente: { ...reserva.paciente, idPaciente: event.target.value } })
              }
              sx={{ marginBottom: '16px' }}
            />
          </Grid>
